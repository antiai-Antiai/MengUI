...
describe('test Input component', () => {
    it('should render the correct default Input', () => {
        ...
        expect(testNode).toHaveClass('meng-input-inner')
        fireEvent.change(testNode, {target: {value: '23'}})
        expect(defaultProps.onChange).toHaveBeenCalled()
        expect(testNode.value).toEqual('23')
    })
    it('should render the disbaled Input on disabled property', () => {
    ...
    it('should render different input sizes on size property', () => {
        ...
        const testContainer = wrapper.container.querySelector('.meng-input-wrapper')
        expect(testContainer).toHaveClass('input-size-lg')
    })
    it('should render prepand and append on prepand/append prpperty', () => {
        const { queryByText, container } = render(<Input placeholder="pend" prepend="https://" append=".com" />)
        const testContainer = container.querySelector('.meng-input-wrapper')
